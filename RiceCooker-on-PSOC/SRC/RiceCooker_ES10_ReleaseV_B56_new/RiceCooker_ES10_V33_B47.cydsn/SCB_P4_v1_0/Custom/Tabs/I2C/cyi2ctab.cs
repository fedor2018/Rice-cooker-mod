...
namespace SCB_P4_v1_0
{
    public partial class CyI2CTab : CyTabControlWrapper
    {
            ...
            : base(parameters)
        {
            ...
            InitializeNumericControlsRanges(m_params.I2C_OvsFactor);

            // Assigning ModeType possible values to ComboBox
            m_cbMode.Items.AddRange(m_params.GetEnumDescriptions(CyParamNames.I2C_MODE));
        ...
        public override void UpdateUI()
        {
            ...
            // I2CMode
            m_cbMode.SelectedItem = m_params.GetValueDescription(CyParamNames.I2C_MODE, m_params.I2C_Mode);
        ...
        public void UpdateClock()
        {
            ...
            if (m_sclk > -1)
            {
                ...
                {
                    dataRateStr = m_actualDataRate.ToString() + " kbps";
                }
            }

            // Update Actual Data Rate label
            m_lblCalcDataRate.Text = dataRateStr;
        }
        ...
        private void UpdateTLowTHigh()
        {
            if (m_actualDataRate > 0)
            {
                ...
                double tLo = Math.Round((1000000 / m_sclk) * m_params.I2C_OvsFactorLow);  // SCLK in nS mult to OvsLow 
                double tHi = Math.Round((1000000 / m_sclk) * m_params.I2C_OvsFactorHigh); // SCLK in nS mult to OvsHigh

                double tLo_ms = Math.Round(tLo / 1000, 3);
                double tHi_ms = Math.Round(tHi / 1000, 3);

                if (m_actualDataRate <= CyParamRanges.I2C_SCL_FREQ_STANDARD_MODE_MAX)
                {
                    if (tLo < (CyParamRanges.I2C_TLO_STANDARD_MODE_MIN * 1000))
                    ...
                    else if (tLo < (CyParamRanges.I2C_THI_STANDARD_MODE_MIN * 1000))
                ...
                {
                    if (tLo < (CyParamRanges.I2C_TLO_FAST_MODE_MIN * 1000))
                        m_errorProvider.SetError(m_numOversamplingFactorLow, string.Format(Resources.TLoError, tLo_ms,
                            CyParamRanges.I2C_TLO_FAST_MODE_MIN, m_actualDataRate));
                    else if (tLo < (CyParamRanges.I2C_THI_FAST_MODE_PLUS_MIN * 1000))
                        m_errorProvider.SetError(m_numOversamplingFactorHigh, string.Format(Resources.THiError, tHi_ms,
                ...
                {
                    if (tLo < (CyParamRanges.I2C_TLO_FAST_MODE_PLUS_MIN * 1000))
                        m_errorProvider.SetError(m_numOversamplingFactorLow, string.Format(Resources.TLoError, tLo_ms,
        ...
        private void tbSlaveAddressMask_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (textboxValue >= CyParamRanges.I2C_SLAVE_ADDRESS_MASK_MIN && textboxValue <=
                    CyParamRanges.I2C_SLAVE_ADDRESS_MASK_MAX)
                {
                    ...
                    throw new Exception(string.Format(Resources.I2CSlaveAddressMaskError, min, max));
        ...
        private void m_cbMode_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                m_params.I2C_Mode = m_params.GetEnumValue<CyEI2CModeType>(CyParamNames.I2C_MODE, m_cbMode.Text);
            }

            bool hasMaster = m_params.I2C_Mode != CyEI2CModeType.I2C_SLAVE;
            bool isSlave = m_params.I2C_Mode == CyEI2CModeType.I2C_SLAVE ||
                m_params.I2C_Mode == CyEI2CModeType.I2C_MULTI_MASTER_SLAVE;

            m_panelSlaveConf.Enabled = isSlave;
            m_numOversamplingFactor.Enabled = hasMaster;
            m_numOversamplingFactorHigh.Enabled = hasMaster;
            m_numOversamplingFactorLow.Enabled = hasMaster;
            m_lblOversamplingFactor.Enabled = hasMaster;
        ...
        private void m_cbDataRate_TextChanged(object sender, EventArgs e)
        {
            ...
            UInt16 dataRateValue = 0;
            bool parsed = UInt16.TryParse(m_cbDataRate.Text, out dataRateValue);

            if (m_params.GlobalEditMode && parsed)
            {
                m_params.I2C_DataRate = dataRateValue;
            }
        ...
        private void m_chb_CheckedChanged(object sender, EventArgs e)
        {
            ...
            else if (sender == m_chbEnableMedianFilter)
            {
                ...
                if (m_errorProvider.GetError(m_numOversamplingFactorLow) == string.Empty &&
                    m_errorProvider.GetError(m_numOversamplingFactorHigh) == string.Empty)
                    InitializeNumericControlsRanges(m_params.I2C_OvsFactor);
            }
        ...
        private void numOversamplingFactor_TextChanged(object sender, EventArgs e)
        {
            ...
            if (m_params.GlobalEditMode)
            {
                ...
                else
                {
                    ...
                    InitializeNumericControlsRanges(ovsFactorValue);
                    ...
                    if (ovsFactorValue != m_previousOvsFactor) // check if I2COvsFactor was changed
                    {
                        ...
                        {
                            m_numOversamplingFactorLow.Value = ovsFactorValue / 2;
                            m_numOversamplingFactorHigh.Value = ovsFactorValue / 2;
                        }
                        else
                        {
        ...
        private void InitializeNumericControlsRanges(byte ovsFactorValue)
        {
            byte min = CyParamRanges.GetI2COvsLowHighMinimum(m_params.I2C_EnableMedianFilter);
            byte max = CyParamRanges.I2C_OVS_LOW_HIGH_MAX;

            m_numOversamplingFactorLow.Minimum = Math.Max(ovsFactorValue - max, min);
            m_numOversamplingFactorLow.Maximum = Math.Min(ovsFactorValue - min, max);
            m_numOversamplingFactorHigh.Minimum = Math.Max(ovsFactorValue - max, min);
            m_numOversamplingFactorHigh.Maximum = Math.Min(ovsFactorValue - min, max);
        }
        ...
        private void numOversamplingFactorLow_Validating(object sender, CancelEventArgs e)
        {
            ...
            if (m_numOversamplingFactorLow.Value < CyParamRanges.GetI2COvsLowHighMinimum(m_params.I2C_EnableMedianFilter))
                m_errorProvider.SetError(m_numOversamplingFactorLow, GetOvsLowErrorMessage());
            else
                m_errorProvider.SetError(m_numOversamplingFactorLow, string.Empty);

            if (m_numOversamplingFactorHigh.Value < CyParamRanges.GetI2COvsLowHighMinimum(
