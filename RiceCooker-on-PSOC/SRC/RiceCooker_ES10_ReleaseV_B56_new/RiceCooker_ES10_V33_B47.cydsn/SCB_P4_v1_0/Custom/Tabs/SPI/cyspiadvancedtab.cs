...
namespace SCB_P4_v1_0
{
    public partial class CySPIAdvancedTab : CyTabControlWrapper
    {
        ...
        {
            bool bufSizeOverflow = (m_params.SPI_RxBufferSize > 8) || (m_params.SPI_TxBufferSize > 8);
            ...
            m_chbInterruptSPIDone.Enabled = interruptEnabled && (m_params.SPI_Mode == CyESPIModeType.SPI_MASTER);
            m_chbInterruptSPIBusError.Enabled = interruptEnabled && (m_params.SPI_Mode == CyESPIModeType.SPI_SLAVE);
            m_chbInterruptTXNotFull.Enabled = interruptEnabled && (m_params.SPI_TxBufferSize == 8);
            ...
            m_chbInterruptRXNotEmpty.Enabled = interruptEnabled && (m_params.SPI_RxBufferSize == 8);
        ...
        void m_numBufferSize_TextChanged(object sender, EventArgs e)
        {
            ...
            if (!ce.Cancel)
            {
                ...
                if (sender == m_numRXBufferSize)
                {
                    ...
                    if (m_params.SPI_RxBufferSize > 8)
                    {
                ...
                if (sender == m_numTXBufferSize)
                {
                    ...
                    if (m_params.SPI_TxBufferSize > 8)
                    {
