    ...
    public partial class CySPIBasicTab : CyTabControlWrapper
    {
        ...
        public bool SimultionesTXRXMode
        {
            get
            {
                ...
                return m_params.SPI_SubMode != CyESPISubModeType.NS;
            }
        }
        public ushort GetHertzMultiplier
        {
            ...
            : base(parameters)
        {
            ...
            m_cbMode.Items.AddRange(m_params.GetEnumDescriptions(CyParamNames.SPI_MODE));
            m_cbSubMode.Items.AddRange(m_params.GetEnumDescriptions(CyParamNames.SPI_SUB_MODE));
            m_cbSCLKMode.Items.AddRange(m_params.GetEnumDescriptions(CyParamNames.SPI_SCLK_MODE));
            m_cbBitOrder.Items.AddRange(m_params.GetEnumDescriptions(CyParamNames.SPI_BITS_ORDER));
        ...
        public override void UpdateUI()
        {
            ...
            m_cbMode.SelectedItem = m_params.GetValueDescription(CyParamNames.SPI_MODE, m_params.SPI_Mode);
            m_cbSubMode.SelectedItem = m_params.GetValueDescription(CyParamNames.SPI_SUB_MODE, m_params.SPI_SubMode);
            m_cbSCLKMode.SelectedItem = m_params.GetValueDescription(CyParamNames.SPI_SCLK_MODE,
                m_params.SPI_SCLKMode);
            m_cbBitOrder.SelectedItem = m_params.GetValueDescription(CyParamNames.SPI_BITS_ORDER,
                m_params.SPI_BitsOrder);
        ...
        private void UpdateCheckBoxesStates()
        {
            m_chbEnableWakeup.Enabled = (m_params.SPI_Mode == CyESPIModeType.SPI_SLAVE) &&
                (m_params.SPI_SubMode == CyESPISubModeType.MOTOROLA) &&
                (m_params.SPI_SCLKMode == CyESPISCLKModeType.MODE_00);
        }
        ...
        public void SetBitrateValue(double bitrate, bool detectUnits)
        {
            ...
            bitrate = bitrate / GetHertzMultiplier;

            object[] list = new object[CyParameters.SPI_DATA_RATE_LIST.Length];
            for (int i = 0; i < list.Length; i++)
            {
                list[i] = (double)CyParameters.SPI_DATA_RATE_LIST[i] / GetHertzMultiplier;
            }
        ...
        void m_cb_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (sender == m_cbMode)
            {
                m_params.SPI_Mode = m_params.GetEnumValue<CyESPIModeType>(CyParamNames.SPI_MODE, m_cbMode.Text);

                bool isMaster = m_params.SPI_Mode == CyESPIModeType.SPI_MASTER;
            ...
            {
                m_params.SPI_SubMode = m_params.GetEnumValue<CyESPISubModeType>(CyParamNames.SPI_SUB_MODE,
                    m_cbSubMode.Text);
                ...
                m_cbSCLKMode.Enabled = m_params.SPI_SubMode == CyESPISubModeType.MOTOROLA;
                switch (m_params.SPI_SubMode)
                {
                    case CyESPISubModeType.TI_COINCIDES:
                    case CyESPISubModeType.TI_PRECEDES:
                        m_cbSCLKMode.SelectedIndex = (int)(CyESPISCLKModeType.MODE_10);
                        break;
                    case CyESPISubModeType.NS:
                        m_cbSCLKMode.SelectedIndex = (int)(CyESPISCLKModeType.MODE_00);
                        break;
                    default:
                        break;
                }
            ...
            {
                m_params.SPI_SCLKMode = m_params.GetEnumValue<CyESPISCLKModeType>(CyParamNames.SPI_SCLK_MODE,
                    m_cbSCLKMode.Text);
                UpdateDrawing();
                UpdateCheckBoxesStates();
            }
            ...
            {
                m_params.SPI_BitsOrder = m_params.GetEnumValue<CyESPIBitOrderType>(CyParamNames.SPI_BITS_ORDER,
                    m_cbBitOrder.Text);
                UpdateDrawing();
            }
        ...
        public void UpdateDrawing()
        {
            ...
            bool normal_mode = ((m_params.SPI_SCLKMode == CyESPISCLKModeType.MODE_00) ||
                  (m_params.SPI_SCLKMode == CyESPISCLKModeType.MODE_01)) ? true : false;
            bool starthigh = (m_params.SPI_SCLKMode == CyESPISCLKModeType.MODE_01) ||
                (m_params.SPI_SCLKMode == CyESPISCLKModeType.MODE_11) ? true : false;
            bool shiftDir = (m_params.SPI_BitsOrder == CyESPIBitOrderType.MSB_FIRST) ? false : true;

            bool nsMode = m_params.SPI_SubMode == CyESPISubModeType.NS;
            ...
            using (Pen extentspen = new Pen(blkbrush))
            {
                ...
                float wfheight = (m_pictDrawing.Height - (2 * PB_EXTENTS_BORDER) - (4 * PB_POLYGON_WIDTH)) / numwaveforms;
                        ...
                        switch (wfnames[i])
                        {
                            ...
                            case "MOSI":
                                ...
                                for (j = 0; j < (mosiEnd); )
                                {
                                    ...
                                    if (nsEndNode == false)
                                    {
                                        ...
                                        wfg.DrawLine(pen, segs[mosiSt + (j + 2)] - 2, highY, segs[mosiSt + (j + 2)] + 2, lowY);
                                        wfg.DrawLine(pen, segs[mosiSt + (j + 2)] - 2, lowY, segs[mosiSt + (j + 2)] + 2, highY);
                                    }
                            ...
                            case "MISO":
                                ...
                                if (nsMode)
                                {
                                    ...
                                    wfg.DrawLine(pen, segs[misoSt + (misoStart)], middle, segs[misoSt + misoStart] + 2, lowY);
                                }
                                else
                                {
                                    // Draw Bus to First Transition Point
                                ...
                                for (j = misoStart; j < (numberOfBits * 2); )
                                {
                                    ...
                                    if (idle == false)
                                        wfg.DrawLine(pen, segs[misoSt + j] + 2, highY, segs[misoSt + (j + 2)] - 2, highY);
                                    wfg.DrawLine(pen, segs[misoSt + j] + 2, lowY, segs[misoSt + (j + 2)] - 2, lowY);

                                    // Draw Transition. For idle node oly bottom line
                                    if (idle)
                                        wfg.DrawLine(pen, segs[misoSt + (j + 2)], middle, segs[misoSt + (j + 2)] + 2, lowY);
                                    else
                                        wfg.DrawLine(pen, segs[misoSt + (j + 2)] - 2, highY, segs[misoSt + (j + 2)] + 2, lowY);
                                    wfg.DrawLine(pen, segs[misoSt + (j + 2)] - 2, lowY, segs[misoSt + (j + 2)] + 2, highY);
                                ...
                                break;
                            case "SCLK":
                                int goHighSeg = m_params.SPI_SubMode == CyESPISubModeType.TI_PRECEDES ? 1 : 3;
                                int addSegments = m_params.SPI_SubMode == CyESPISubModeType.TI_PRECEDES ? 1 : 0;

                                wfg.DrawLine(pen, segs[0], starthigh ? highY : lowY, segs[goHighSeg], starthigh ? highY : lowY);
                            ...
                            case "Sample":
                                ...
                                for (j = 0; j < (numberOfBits * 2); )
                                {
                                    ...
                                    if (j + 2 < numberOfBits * 2)
                                        wfg.DrawLine(pen, segs[misoSt + 1 + j] + 2, lowY, segs[misoSt + 1 + (j + 2)] - 2, lowY);
                                    else
