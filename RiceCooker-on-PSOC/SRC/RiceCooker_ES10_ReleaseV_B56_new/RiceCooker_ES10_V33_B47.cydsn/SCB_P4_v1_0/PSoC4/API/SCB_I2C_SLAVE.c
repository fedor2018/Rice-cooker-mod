...
volatile uint32  `$INSTANCE_NAME`_slOverFlowCount;  /* Slave Transmit Overflow counter */
...
uint32 `$INSTANCE_NAME`_I2CSlaveClearReadStatus(void)
{
    ...
    status = (`$INSTANCE_NAME`_slStatus & `$INSTANCE_NAME`_I2C_SSTAT_RD_MASK);
    `$INSTANCE_NAME`_slStatus &= ((uint32) ~`$INSTANCE_NAME`_I2C_SSTAT_RD_CLEAR);
...
uint32 `$INSTANCE_NAME`_I2CSlaveClearWriteStatus(void)
{
    ...
    /* Mask of transfer complete flag and Error status */
    status = (`$INSTANCE_NAME`_slStatus & `$INSTANCE_NAME`_I2C_SSTAT_WR_MASK);
    `$INSTANCE_NAME`_slStatus &= ((uint32) ~`$INSTANCE_NAME`_I2C_SSTAT_WR_CLEAR);
...
{
    `$INSTANCE_NAME`_RX_MATCH_REG &= ((uint32) ~`$INSTANCE_NAME`_RX_MATCH_ADDR__MASK);     /* Clear address bits */
    `$INSTANCE_NAME`_RX_MATCH_REG |= `$INSTANCE_NAME`_GET_I2C_8BIT_ADDRESS(address);
}
...
{
    `$INSTANCE_NAME`_RX_MATCH_REG &= ((uint32) ~`$INSTANCE_NAME`_RX_MATCH_MASK__MASK);     /* Clear address mask bits */
    `$INSTANCE_NAME`_RX_MATCH_REG |= ((uint32) (mask  << `$INSTANCE_NAME`_RX_MATCH_MASK__POS));
}
...
void `$INSTANCE_NAME`_I2CSlaveInitReadBuf(uint8 * rdBuf, uint32 bufSize)
{
    ...
    if(NULL != rdBuf)
    {
        ...
        `$INSTANCE_NAME`_slRdBufIndex = 0ul;                      /* Clears buffer index */
    }
}
...
void `$INSTANCE_NAME`_I2CSlaveInitWriteBuf(uint8 * wrBuf, uint32 bufSize)
{
    ...
    if(NULL != wrBuf)
    {
        ...
        `$INSTANCE_NAME`_slWrBufIndex = 0ul;                      /* Clears buffer index */
    }
}
...
{
    `$INSTANCE_NAME`_slRdBufIndex = 0ul;
}
...
{
    `$INSTANCE_NAME`_slWrBufIndex = 0ul;
}
