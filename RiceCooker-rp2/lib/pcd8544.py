...
from machine import Pin, SPI, SoftSPI, PWM
from array import array
import utime, time
from ustruct import pack

class PCD8544():
    def __init__(self, spi_id=None, rst=None, ce=None, dc=None, din=None, clk=None, dout=None, pwm=None):
        self._rst = Pin(rst, Pin.OUT) if rst else None # 14
        self._pwm = PWM(Pin(pwm)) if pwm else None # 15
...
#         self.LInit = bytearray(b'\x21\xbc\x04\x14\x20\x0C') #contast, TCx, BSx,Normal  
    ...
    def pwm(self, d=50, f=0):
        """ d=duty in % """
        if self._pwm is not None:
            if f: self._pwm.freq(f)
            self._pwm.duty_u16(int((65635/100)*d))
#             print("{} {}".format(self._pwm.duty_u16(),self._pwm.freq()))
